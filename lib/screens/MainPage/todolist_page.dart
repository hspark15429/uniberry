...
class _TodolistPageState extends State<TodolistPage> {
  ...
  Color appBarColor = Color.fromARGB(255, 7, 173, 135);
  ...
  void initState() {
   ...
   Map<DateTime, List<TodoItem>> _groupEvents(List<TodoItem> events) {
    ...
    for (var event in events) {
      DateTime start = DateTime(event.date.year, event.date.month, event.date.day);
      DateTime? end = event.endDate != null
          ? DateTime(event.endDate!.year, event.endDate!.month, event.endDate!.day)
          : null;
      DateTime currentDate = start;
      while (end == null || currentDate.compareTo(end) <= 0) {
        if (data[currentDate] == null) data[currentDate] = [];
        data[currentDate]!.add(event);
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    decoration: InputDecoration(labelText: '내용 (선택 사항)'),
                    ...
                    onTap: () async {
                      ...
                      if (pickedDate != null) {
                        setState(() {
                          ...
                          selectedDateRange = null; // 날짜 선택 시 범위 선택 초기화
                        });
                      }
                    },
                  ),
